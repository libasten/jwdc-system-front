<template>
  ...
  <div class="app-container">
    <div class="top-btns">
      <el-button-group>
        <el-button type="primary" size="small" icon="el-icon-plus" @click="createNoteType">新建</el-button>
        <el-button v-if="currentRow!=null" type="primary" size="small" icon="el-icon-edit" @click="editNoteType">编辑</el-button>
        <el-button v-if="currentRow!=null" type="primary" size="small" icon="el-icon-reading" @click="cancelSelected">取消选中</el-button>
        <el-button v-if="currentRow!=null" type="danger" size="small" icon="el-icon-delete" @click="deleteNoteType">删除</el-button>
      </el-button-group>
    </div>
    <div class="table-view">
      <el-table v-loading="listLoading" ref="vTable" :data="list.slice((currentPage-1)*pageSize,currentPage*pageSize)" @current-change="handleCurrentChange" border fit stripe highlight-current-row :header-cell-style="headerCellStyle" :cell-style="columnStyle">
        ...
        <el-table-column min-width="60%" label="备注类型" header-align="center" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <span>{{ row.name }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog title="项目附件类型" :visible.sync="dialogVisible" :close-on-click-modal="false" width="50%">
      <el-form ref="postForm" :model="postForm" :rules="rules" label-width="80px">
        ...
        <el-form-item label="备注类型" prop="name">
...
import { fetchProjectNoteType, editProjectNoteType, createProjectNoteType, delProjectNoteType } from '@/api/project';
  ...
  methods: {
    submit() {
      this.$refs.postForm.validate((valid) => {
        if (valid) {
          // 新建
          if (this.postForm.id === '') {
            createProjectNoteType(this.postForm).then((res) => {
          ...
          // 编辑更新
            ...
            editProjectNoteType(this.postForm).then((res) => {
    ...
    deleteNoteType() {
      this.$confirm('永久删除, 是否继续?', '提示', {
        ...
        .then(() => {
          delProjectNoteType(this.currentRow).then((res) => {
