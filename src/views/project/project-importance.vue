  ...
  <div class="app-container">
    <div class="top-btns">
      <el-button-group>
        ...
        <el-button v-if="currentRow!=null" type="primary" size="small" icon="el-icon-edit" @click="editImportance">编辑</el-button>
    ...
    <div class="table-view">
      <el-table v-loading="listLoading" ref="vTable" :data="list.slice((currentPage-1)*pageSize,currentPage*pageSize)" @current-change="handleCurrentChange" border fit stripe highlight-current-row :header-cell-style="heaerCellStyle" :cell-style="columnStyle">
        ...
        <el-table-column min-width="60%" label="难易程度" header-align="center" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <span>{{ row.name }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog title="项目难易程度" :visible.sync="dialogVisible" :close-on-click-modal="false" width="50%">
...
import { fetchProjectImportance, editProjectImportance, createProjectImportance, delProjectImportance } from '@/api/project';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.list = [];
      fetchProjectImportance().then((res) => {
